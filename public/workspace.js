(() => {
  ...
  let route = 'home';
  let csrf = '';
  let saving = false, saveAgain = false, conflict = false;
  let memberId = '';
  let snapshot = null;
  let saveTimer = null;
  ...
  async function api(path, options = {}) {
    const res = await fetch(path, { ...options, credentials:'same-origin', headers:{'Content-Type':'application/json','X-Onebiz-Client':'web','X-CSRF-Token':csrf} });
    const body = await res.json();
    if (res.status === 401) { location.replace('/'); throw new Error('로그인이 필요합니다.'); }
    if (!res.ok) { const error = new Error(body.error || '저장 실패'); error.status=res.status; throw error; }
    return body;
  }
  async function loadSnapshot() {
    if (saving) { showToast('저장 완료 후 새로고침하세요.'); return; }
    if ((conflict || saveTimer) && !confirm('현재 화면의 미저장 내용을 버리고 서버 데이터를 불러올까요?')) return;
    clearTimeout(saveTimer);saveTimer=null;
    setSync('busy','불러오는 중');
    const body = await api('/api/snapshot'); snapshot=normalizeSnapshot(body); memberId=body.userId;
    $('#sidebarMemberId').textContent=memberId;conflict=false;setSync('ok','서버 버전 '+body.revision);render();
  }
  async function saveSnapshot() {
    saveTimer=null;
    if (!snapshot || conflict) return;
    if (saving) { saveAgain=true; return; }
    saving=true;setSync('busy','저장 중');
    try {
      const sent = JSON.stringify({revision:snapshot.revision,data:snapshot.data,employees:snapshot.employees});
      const body = await api('/api/snapshot',{method:'PUT',body:sent});
      snapshot.revision=body.revision;snapshot.updatedAt=Date.now();setSync('ok','서버 버전 '+body.revision);showToast('서버에 저장했습니다.');
    } catch(e) {
      conflict=true;setSync('error','미저장 · 확인 필요');showToast(e.message+' 새로고침 전에 미저장 내용을 확인해 주세요.');
    } finally {
      saving=false;
      if(saveAgain && !conflict){saveAgain=false;saveTimer=setTimeout(saveSnapshot,250);}
    }
  }
  function queueSave(){clearTimeout(saveTimer);saveTimer=setTimeout(saveSnapshot,250);}
  function mutate(message){render();if(conflict){showToast('저장 충돌 상태입니다. 서버에서 다시 불러온 후 수정하세요.');return;}queueSave();showToast('변경 내용을 저장하는 중입니다.');}
  ...
  function openQuoteModal(quote = null, presetCustomer = null) {
    ...
    openModal(quote?'견적서 수정':'새 견적서 작성',`${draft.id} · A4 견적서 출력 지원`, `
      <div class="form-grid">
        ...
        <div class="item-editor"><div class="item-head"><span>품목명</span><span>규격</span><span>수량</span><span>단가</span><span></span></div><div id="quoteItems"></div><div class="v2layout0"><button type="button" id="addQuoteItem" class="secondary">+ 품목 추가</button></div></div>
  ...
  function openContractModal(contract = null, newlyConverted = false) {
    if(contract?.signed){showToast('체결된 계약 원문은 웹에서 변경하지 않습니다. 대금 메뉴에서 입금만 기록하세요.');return;}
    ...
    openModal(newlyConverted?'견적 → 계약 전환':(contract?'계약서 수정':'계약서 작성'), `${c.id} · 전자서명 발송은 Android 앱에서도 이어서 사용할 수 있습니다.`, `<div class="form-grid">
      ...
      <div class="field"><label>시공예정일<input id="cWorkDate" type="date" value="${esc(c.workDate||'')}"></label></div><div class="field"><label>서명상태<select id="cSigned" disabled><option value="false">서명대기 (별도 서명 절차 필요)</option></select></label></div>
  ...
  async function logout(){
    if ((saving || saveTimer || conflict) && !confirm('미저장 작업이 있을 수 있습니다. 로그아웃할까요?'))return;
    try{await api('/api/logout',{method:'POST',body:'{}'});location.assign('/');}catch(e){showToast(e.message);}
  }
  $('#logoutBtn').onclick=()=>logout();
  $('#refreshBtn').onclick=()=>loadSnapshot().catch(e=>showToast(e.message));
  $('#profileBtn').onclick=()=>{route='settings';render();};
  async function boot(){try{const me=await api('/api/me');csrf=me.csrf;if(!me.user.profileComplete){location.replace('/');return;}memberId=me.user.id;appShell.classList.remove('hidden');await loadSnapshot();}catch(e){showToast(e.message);}}
  window.addEventListener('beforeunload',e=>{if(saving||saveTimer||conflict){e.preventDefault();e.returnValue='';}});
  boot();
})();
